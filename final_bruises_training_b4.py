...
def plot_class_distribution(dataset, title):
    ...
    save_path = os.path.join(data_dir,"final", f"{title.lower()}_class_distribution_bruises.png")
    plt.tight_layout()
...
def evaluate_model(model, test_loader, class_names):
    ...
    save_path = os.path.join(data_dir, "final",  "confusion_matrix_bruises.png")
    plt.tight_layout()
    plt.savefig(save_path)
    plt.close()
    print(f"Saved plot: {save_path}")
...
def train_model(num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for images, labels in progress_bar:
            ...
            # AUTOCAST & SCALER (updated to torch.amp API)
    ...
    plt.legend()
    plt.grid(True)
    save_path = os.path.join(data_dir,"final", "val_loss_accuracy_curve_bruises.png")
    plt.tight_layout()
    plt.savefig(save_path)
    plt.close()
    print(f"Saved plot: {save_path}")
    ...
    # Save best model
    file_path = os.path.join(data_dir,"final", "bruises_b4.pth")
...
def main():
    EPOCHS = 30
    log_path = os.path.join(data_dir,"final", "log_bruises.txt")
