...
def plot_class_distribution(dataset, title):
    ...
    save_path = os.path.join(data_dir,"efficientnet-b1", f"{title.lower()}_class_distribution_ripeness_effnet-b1.png")
    plt.tight_layout()
...
def evaluate_model(model, test_loader, class_names):
    ...
    save_path = os.path.join(data_dir,"efficientnet-b1", "confusion_matrix_ripeness_effnet-b1.png")
    plt.tight_layout()
    plt.savefig(save_path)
    plt.close()
    print(f"Saved plot: {save_path}")
...
def train_model(num_epochs):
    ...
    model = EfficientNet.from_pretrained('efficientnet-b1', num_classes=len(class_names))
    ...
    plt.legend()
    plt.grid(True)
    save_path = os.path.join(data_dir,"efficientnet-b1", "val_loss_accuracy_curve_ripeness_effnet-b1.png")
    plt.tight_layout()
    plt.savefig(save_path)
    plt.close()
    print(f"Saved plot: {save_path}")

    file_path = os.path.join(data_dir, "ripeness_b1.pth")
...
def main():
    # CHANGE ME
    EPOCHS = 15
    log_path = os.path.join(data_dir,"efficientnet-b1", "log_ripeness_effnet-b1.txt")
