...
# ---- TRANSFORMS (EfficientNetV2-M expects 480x480) ----
IMG_SIZE = 480
...
def plot_class_distribution(dataset, title):
    ...
    save_path = os.path.join(data_dir, f"{title.lower()}_class_distribution_ripeness.png")
    plt.tight_layout()
...
def evaluate_model(model, test_loader, class_names):
    ...
    save_path = os.path.join(data_dir, "confusion_matrix_ripeness.png")
    plt.tight_layout()
    plt.savefig(save_path)
    plt.close()
    print(f"Saved plot: {save_path}")
...
def train_model(num_epochs, patience=10):
    ...
    plt.legend()
    plt.grid(True)
    save_path = os.path.join(data_dir, "val_loss_accuracy_curve_ripeness.png")
    plt.tight_layout()
    plt.savefig(save_path)
    plt.close()
    print(f"Saved plot: {save_path}")
    ...
    # ---- Save best model ----
    file_path = os.path.join(data_dir, "ripeness_v2m.pth")
